...
const FONT_FAMILIES = [
  {
    value: '"Geist Mono Variable", ui-monospace, SFMono-Regular, Menlo, Consolas, monospace',
...
const EDITOR_TOGGLES = [
  { key: 'isLineNumbersDisplayed',     label: 'Line numbers',           hint: 'Show the gutter with line numbers.' },
  { key: 'isLineWrappingEnabled',      label: 'Line wrapping',          hint: 'Soft-wrap long lines to fit the editor width.' },
  { key: 'isBracketMatchingEnabled',   label: 'Bracket matching',       hint: 'Highlight the matching bracket when the cursor is on one.' },
  { key: 'isBracketClosingEnabled',    label: 'Auto-close brackets',    hint: 'Insert the closing bracket automatically.' },
  { key: 'isActiveLineHighlighted',    label: 'Active line highlight',  hint: 'Tint the line the cursor is on.' },
  { key: 'isPatternHighlightingEnabled', label: 'Pattern highlighting', hint: 'Outline the notes currently being played in the pattern.' },
  { key: 'isFlashEnabled',             label: 'Eval flash',             hint: 'Flash the editor background when a pattern is re-evaluated.' },
  { key: 'isTabIndentationEnabled',    label: 'Tab indentation',        hint: 'Use Tab to indent instead of the default behavior.' },
  { key: 'isMultiCursorEnabled',       label: 'Multi-cursor',           hint: 'Allow Cmd/Ctrl-click to add additional cursors.' },
];
...
export function createSettingsPanel({
  ...
  function buildHeader() {
    ...
    const title = el('div', 'settings-panel__title', 'Settings');
  ...
  function buildAppearanceSection() {
    ...
    // ─── Accent color: hue + lightness sliders ─────────────────────
    // Two rows so the labels line up with the rest of the section.
    ...
    hueRow.control.appendChild(hueInput);
    ...
    lightRow.control.appendChild(lightInput);
    section.body.appendChild(lightRow.row);

    // Swatch + reset link — shares a grid row with the other appearance
    // controls so the reset link's left edge aligns with the other
    // inputs. The swatch sits in the label column; the control column
    // holds the reset link.
    const swatchRow = buildRow('');
    accentSwatch = el('div', 'settings-panel__swatch');
    accentSwatch.setAttribute('aria-hidden', 'true');
    swatchRow.row.querySelector('.settings-panel__row-label').replaceWith(
      accentSwatch,
    );
    accentResetBtn = el('button', 'settings-panel__reset', 'Reset to default');
    accentResetBtn.type = 'button';
    accentResetBtn.addEventListener('click', onAccentResetClick);
    swatchRow.control.appendChild(accentResetBtn);
    section.body.appendChild(swatchRow.row);
  ...
  function buildEditorSection() {
    ...
    for (const { key, label, hint } of EDITOR_TOGGLES) {
      ...
      btn.addEventListener('click', () => onToggleClick(key));
