...
export function createConsolePanel({
  ...
  function create(container) {
    ...
    for (const lvl of LEVELS) {
      ...
      btn.type = 'button';
      btn.className =
        `console-panel__level console-panel__level--${lvl.id} is-active`;
  ...
  function pushEntry(level, rawMessage, data) {
    ...
    while (entries.length > MAX_ENTRIES) {
      ...
      if (mounted) {
        const stale = listEl.querySelector(
          `[data-entry-id="${removed.id}"]`,
        );
  ...
  function buildHapCard(entry, hap) {
    ...
    span.textContent =
      `span ${fmtFraction(hap.whole?.begin)} → ${fmtFraction(hap.whole?.end)}`;
